...
//       await execute(
//         `UPDATE demo_requests SET assignment_status='assigned',
//          assigned_tutor_id=?, assigned_at=NOW(), updated_at=NOW() WHERE id=?`,
//         [tutorId, id]
//       );
//       await execute(
...
//       if (!tutorId) return NextResponse.json({ error: "tutorId required" }, { status: 400 });
//       await execute(
//         `UPDATE demo_requests SET assignment_status='reassigned',
//          assigned_tutor_id=?, assigned_at=NOW(), updated_at=NOW() WHERE id=?`,
//         [tutorId, id]
//       );
//       await execute(
