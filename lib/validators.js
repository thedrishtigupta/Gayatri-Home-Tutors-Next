...
export function validateDemoRequest(body) {
  ...
  // The tutor this family asked for by name, if they arrived from a profile.
  // Shape only is checked here; whether the tutor exists and is still active is
  // settled against the database in the route, which is the only place that can
  // actually know.
  const rawRequested = Number.parseInt(body?.requestedTutorId, 10);
  const requestedTutorId = Number.isInteger(rawRequested) && rawRequested > 0 ? rawRequested : null;
  ...
  return {
    ...
    errors,
    data: { fullName, phone, email: email || null, message: message || null, area: area || null, studentClass: studentClass || null, time, source, subjects, requestedTutorId },
  };
}
