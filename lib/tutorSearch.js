...
/**
 * One tutor, in the same public-safe shape a search result has.
 *
 * Used by /book-demo?tutor=<id> to confirm who the family is asking for. It
 * applies the same LIVE filter as search on purpose: a tutor who has been
 * deactivated must not be requestable through a stale or hand-edited link.
 *
 * Returns null rather than throwing when the id is unknown or inactive — the
 * caller falls back to the ordinary form, because a booking is far too
 * valuable to lose over a bad query string.
 */
export async function getTutorPublicById(id) {
  const tutorId = toInt(id);
  ...
  const [subjectRows, classRows, areaRows] = await Promise.all([
    query(
      `SELECT DISTINCT p.tutor_id, s.name, s.slug
       FROM tutor_teaching_profiles p JOIN subjects s ON s.id = p.subject_id
       WHERE p.tutor_id = ? ORDER BY s.name`,
      [tutorId]
    ),
    query(
      `SELECT DISTINCT p.tutor_id, c.name, c.slug, c.sort_order
       FROM tutor_teaching_profiles p JOIN classes c ON c.id = p.class_id
       WHERE p.tutor_id = ? ORDER BY c.sort_order`,
      [tutorId]
    ),
    query(
      `SELECT DISTINCT tl.tutor_id, l.name, l.slug
  ...
  return shapeTutor(rows[0], group(subjectRows), group(classRows), group(areaRows));
}
