...
export default function TutorsPage() {
  ...
  useEffect(() => {
    setPage(1);
  }, [debouncedSearch, filters.subjectId, filters.classId, filters.areaId, filters.status, filters.gender, filters.verified, filters.profileCompleted]);
  ...
  async function updateTutor(id, patch) {
    ...
    if (!ok) {
      ...
      setTutors((rows) => rows.map((t) => (t.id === id ? { ...t, ...data } : t)));
    }
